/* Generated by Nim Compiler v2.0.4 */
#define NIM_INTBITS 64
#define NIM_EmulateOverflowChecks

#include "nimbase.h"
#include <string.h>
#undef LANGUAGE_C
#undef MIPSEB
#undef MIPSEL
#undef PPC
#undef R3000
#undef R4000
#undef i386
#undef linux
#undef mips
#undef near
#undef far
#undef powerpc
#undef unix
typedef struct tyObject_Result__GLXz9aENXQdHR5vUaWfCInQ tyObject_Result__GLXz9aENXQdHR5vUaWfCInQ;
typedef struct tySequence__6H5Oh5UUvVCLiakt9aTwtUQ tySequence__6H5Oh5UUvVCLiakt9aTwtUQ;
typedef struct tySequence__6H5Oh5UUvVCLiakt9aTwtUQ_Content tySequence__6H5Oh5UUvVCLiakt9aTwtUQ_Content;
struct tySequence__6H5Oh5UUvVCLiakt9aTwtUQ {
  NI len; tySequence__6H5Oh5UUvVCLiakt9aTwtUQ_Content* p;
};
struct tyObject_Result__GLXz9aENXQdHR5vUaWfCInQ {
	NIM_BOOL oResultPrivate;
union{
struct {	NCSTRING eResultPrivate;
} _oResultPrivate_1;
struct {	tySequence__6H5Oh5UUvVCLiakt9aTwtUQ vResultPrivate;
} _oResultPrivate_2;
};
};
struct tySequence__6H5Oh5UUvVCLiakt9aTwtUQ_Content { NI cap; NU8 data[SEQ_DECL_SIZE]; };
static N_INLINE(NI, emarkamp___pureZhashes_u4)(NI h_p0, NI val_p1);
static N_INLINE(NI, hash__gif50anim_u365)(NIM_BOOL x_p0);
static N_INLINE(NI, hashWangYi1__pureZhashes_u139)(NU64 x_p0);
static N_INLINE(NU64, hiXorLo__pureZhashes_u80)(NU64 a_p0, NU64 b_p1);
N_LIB_PRIVATE N_NIMCALL(NI, hash__pureZhashes_u303)(NCSTRING x_p0);
N_LIB_PRIVATE N_NIMCALL(NI, murmurHash__pureZhashes_u195)(NU8* x_p0, NI x_p0Len_0);
static N_INLINE(void, copyMem__system_u1740)(void* dest_p0, void* source_p1, NI size_p2);
static N_INLINE(void, nimCopyMem)(void* dest_p0, void* source_p1, NI size_p2);
static N_INLINE(NU32, rotl32__pureZhashes_u183)(NU32 x_p0, NI r_p1);
static N_INLINE(NI, nimCStrLen)(NCSTRING a_p0);
N_LIB_PRIVATE N_NIMCALL(NI, hash__gif50anim_u375)(NU8* x_p0, NI x_p0Len_0);
static N_INLINE(NI, emarkdollar___pureZhashes_u19)(NI h_p0);
static N_INLINE(NI, emarkamp___pureZhashes_u4)(NI h_p0, NI val_p1) {
	NI result;
	NU h_2;
	NU val_2;
	NU res;
	h_2 = ((NU) (h_p0));
	val_2 = ((NU) (val_p1));
	res = (NU)((NU64)(h_2) + (NU64)(val_2));
	res = (NU)((NU64)(res) + (NU64)((NU)((NU64)(res) << (NU64)(((NI)10)))));
	res = (NU)(res ^ (NU)((NU64)(res) >> (NU64)(((NI)6))));
	result = ((NI) (res));
	return result;
}
static N_INLINE(NU64, hiXorLo__pureZhashes_u80)(NU64 a_p0, NU64 b_p1) {
	NU64 result;
	result = (NU64)0;
	__uint128_t r = a_p0; r *= b_p1; result = (r >> 64) ^ r;
	return result;
}
static N_INLINE(NI, hashWangYi1__pureZhashes_u139)(NU64 x_p0) {
	NI result;
	NU64 T1_;
	NU64 T2_;
	result = (NI)0;
	T1_ = (NU64)0;
	T1_ = hiXorLo__pureZhashes_u80(11562461410679940143ULL, (NU64)(x_p0 ^ 16646288086500911323ULL));
	T2_ = (NU64)0;
	T2_ = hiXorLo__pureZhashes_u80(T1_, 16952864883938283885ULL);
	result = ((NI) (T2_));
	return result;
}
static N_INLINE(NI, hash__gif50anim_u365)(NIM_BOOL x_p0) {
	NI result;
	result = hashWangYi1__pureZhashes_u139(((NU64) (x_p0)));
	return result;
}
static N_INLINE(void, nimCopyMem)(void* dest_p0, void* source_p1, NI size_p2) {
	void* T1_;
	T1_ = (void*)0;
	T1_ = memcpy(dest_p0, source_p1, ((size_t) (size_p2)));
}
static N_INLINE(void, copyMem__system_u1740)(void* dest_p0, void* source_p1, NI size_p2) {
	nimCopyMem(dest_p0, source_p1, size_p2);
}
static N_INLINE(NU32, rotl32__pureZhashes_u183)(NU32 x_p0, NI r_p1) {
	NU32 result;
	result = (NU32)((NU32)((NU64)(x_p0) << (NU64)(r_p1)) | (NU32)((NU32)(x_p0) >> (NU64)((NI)(((NI)32) - r_p1))));
	return result;
}
N_LIB_PRIVATE N_NIMCALL(NI, murmurHash__pureZhashes_u195)(NU8* x_p0, NI x_p0Len_0) {
	NI result;
	NI size;
	NI stepSize;
	NI n;
	NU32 h1;
	NI i;
	NU32 k1_2;
	NI rem;
{	size = x_p0Len_0;
	stepSize = ((NI)4);
	n = (NI)(size / stepSize);
	h1 = (NU32)0;
	i = ((NI)0);
	{
		while (1) {
			NU32 k1;
			if (!(i < (NI)(n * stepSize))) goto LA2;
			k1 = (NU32)0;
			copyMem__system_u1740(((void*) ((&k1))), ((void*) ((&x_p0[i]))), ((NI)4));
			i += stepSize;
			k1 = (NU32)((NU32)(k1) * (NU32)(((NU32)IL64(3432918353))));
			k1 = rotl32__pureZhashes_u183(k1, ((NI)15));
			k1 = (NU32)((NU32)(k1) * (NU32)(((NU32)461845907)));
			h1 = (NU32)(h1 ^ k1);
			h1 = rotl32__pureZhashes_u183(h1, ((NI)13));
			h1 = (NU32)((NU32)((NU32)((NU32)(h1) * (NU32)(((NU32)5)))) + (NU32)(((NU32)IL64(3864292196))));
		} LA2: ;
	}
	k1_2 = (NU32)0;
	rem = (NI)(size % stepSize);
	{
		while (1) {
			if (!(((NI)0) < rem)) goto LA4;
			rem -= ((NI)1);
			k1_2 = (NU32)((NU32)((NU64)(k1_2) << (NU64)(((NI)8))) | ((NU32) (x_p0[(NI)(i + rem)])));
		} LA4: ;
	}
	k1_2 = (NU32)((NU32)(k1_2) * (NU32)(((NU32)IL64(3432918353))));
	k1_2 = rotl32__pureZhashes_u183(k1_2, ((NI)15));
	k1_2 = (NU32)((NU32)(k1_2) * (NU32)(((NU32)461845907)));
	h1 = (NU32)(h1 ^ k1_2);
	h1 = (NU32)(h1 ^ ((NU32) (size)));
	h1 = (NU32)(h1 ^ (NU32)((NU32)(h1) >> (NU64)(((NI)16))));
	h1 = (NU32)((NU32)(h1) * (NU32)(((NU32)IL64(2246822507))));
	h1 = (NU32)(h1 ^ (NU32)((NU32)(h1) >> (NU64)(((NI)13))));
	h1 = (NU32)((NU32)(h1) * (NU32)(((NU32)IL64(3266489909))));
	h1 = (NU32)(h1 ^ (NU32)((NU32)(h1) >> (NU64)(((NI)16))));
	result = ((NI) (h1));
	goto BeforeRet_;
	}BeforeRet_: ;
	return result;
}
static N_INLINE(NI, nimCStrLen)(NCSTRING a_p0) {
	NI result;
	size_t T1_;
	T1_ = (size_t)0;
	T1_ = strlen(a_p0);
	result = ((NI) (T1_));
	return result;
}
N_LIB_PRIVATE N_NIMCALL(NI, hash__pureZhashes_u303)(NCSTRING x_p0) {
	NI result;
	result = murmurHash__pureZhashes_u195((NU8*)(x_p0)+(((NI)0)), ((x_p0 ? (nimCStrLen(x_p0)-1) : -1))-(((NI)0))+1);
	return result;
}
N_LIB_PRIVATE N_NIMCALL(NI, hash__gif50anim_u375)(NU8* x_p0, NI x_p0Len_0) {
	NI result;
	result = murmurHash__pureZhashes_u195(x_p0, x_p0Len_0);
	return result;
}
static N_INLINE(NI, emarkdollar___pureZhashes_u19)(NI h_p0) {
	NI result;
	NU h_2;
	NU res;
	h_2 = ((NU) (h_p0));
	res = (NU)((NU64)(h_2) + (NU64)((NU)((NU64)(h_2) << (NU64)(((NI)3)))));
	res = (NU)(res ^ (NU)((NU64)(res) >> (NU64)(((NI)11))));
	res = (NU)((NU64)(res) + (NU64)((NU)((NU64)(res) << (NU64)(((NI)15)))));
	result = ((NI) (res));
	return result;
}
N_LIB_PRIVATE N_NIMCALL(NI, hash__gif50anim_u357)(tyObject_Result__GLXz9aENXQdHR5vUaWfCInQ x_p0) {
	NI result;
	NI T1_;
	result = ((NI)0);
	T1_ = (NI)0;
	T1_ = hash__gif50anim_u365(x_p0.oResultPrivate);
	result = emarkamp___pureZhashes_u4(result, T1_);
	switch (x_p0.oResultPrivate) {
	case NIM_FALSE:
	{
		NI T3_;
		T3_ = (NI)0;
		T3_ = hash__pureZhashes_u303(x_p0._oResultPrivate_1.eResultPrivate);
		result = emarkamp___pureZhashes_u4(result, T3_);
	}
	break;
	case NIM_TRUE:
	{
		NI T5_;
		T5_ = (NI)0;
		T5_ = hash__gif50anim_u375(((x_p0._oResultPrivate_2.vResultPrivate).p) ? (x_p0._oResultPrivate_2.vResultPrivate.p->data) : NIM_NIL, x_p0._oResultPrivate_2.vResultPrivate.len);
		result = emarkamp___pureZhashes_u4(result, T5_);
	}
	break;
	default:
	{
	}
	break;
	}
	result = emarkdollar___pureZhashes_u19(result);
	return result;
}
